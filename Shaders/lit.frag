...
void main() {
    ...
    // cubemap pixel × 3× auto-exposure shows as a 12 linear glint, then
    ...
    // Pre-TAA firefly clamp. The 12.0 default was tuned when exposure was
    // a fixed 1×; now that auto-exposure (qualityToggles2.x) routinely
    // multiplies the lit signal by 2–4× before tonemap, a 12.0 HDR pixel
    // displays as 24–48 — well into AgX's white-crush zone, but each such
    // pixel ALSO accumulates as a TAA history spike. Dropping to 5.0
    // bounds the displayed peak (after typical auto-exposure ≤ 2.5×) at
    // ~12.5 linear, which AgX rolls off cleanly without blowing out, AND
    // stops sub-pixel spec sparkle from feeding the TAA history at all.
    lighting = min(lighting, vec3(5.0));
