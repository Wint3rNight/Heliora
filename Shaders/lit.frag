...
void main() {
    ...
    // Pre-TAA firefly clamp. Bright sub-pixel spec spikes from normal-map
    // detail × HDR sun intensity (~5+) were feeding unclamped into TAA
    // history, and the 3×3 YCoCg AABB in second.frag is too tight to reject
    // them when the spike *was* generated by the current frame's rendering.
    // Clamping to 12.0 stops a single firefly from accumulating into a
    // persistent ghost while leaving honest highlights (white plaster
    // ~1-3, gold trim ~3-8) untouched. Tune up if you want more highlight
    // sparkle, down if glitter persists.
    lighting = min(lighting, vec3(12.0));
