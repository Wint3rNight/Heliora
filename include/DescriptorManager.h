...
public:
  ...
  void init(VkDevice device, VmaAllocator allocator, size_t swapchainImageCount);
  ...
  VkDescriptorSet getSamplerSet(size_t i) const { return samplerDescriptorSets[i]; }
  VkDescriptorSet getGBufferSet(size_t i) const { return gBufferDescriptorSets[i]; }
  ...
  int createTextureDescriptor(VkDevice device,
                              VkImageView albedo, VkImageView normal,
                              VkImageView metallic, VkImageView roughness,
                              VkImageView ao, VkSampler sampler);
  ...
  void updateShadowMapDescriptor(VkDevice device, VkImageView shadowView,
                                 VkImageView pointShadowView, VkSampler sampler);

  // Writes IBL + SSAO noise + skybox into every VP descriptor set (bindings 3-7).
  void updateIblDescriptors(VkDevice device,
                             VkImageView irradianceView, VkSampler iblSampler,
                             VkImageView prefilteredView,
                             VkImageView brdfLutView,
                             VkImageView ssaoNoiseView, VkSampler noiseSampler,
...
private:
  // --- Layouts ---
  VkDescriptorSetLayout descriptorSetLayout = VK_NULL_HANDLE; // VP / scene (set 0)
  VkDescriptorSetLayout samplerSetLayout    = VK_NULL_HANDLE; // 5 PBR textures (set 1, geometry)
  VkDescriptorSetLayout gBufferSetLayout    = VK_NULL_HANDLE; // 4 G-buffer samplers (set 1, deferred)
  VkDescriptorSetLayout inputSetLayout      = VK_NULL_HANDLE; // 1 input attachment (set 0, post-process)
  ...
  void createInputDescriptorSets(VkDevice device, const VulkanSwapchain &swapchain);
};
