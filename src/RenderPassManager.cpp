...
void RenderPassManager::createGBufferRenderPass(VkDevice device,
  ...
  if (vkCreateRenderPass(device, &ci, nullptr, &gBufferRenderPass) != VK_SUCCESS)
...
void RenderPassManager::createRenderPass(VkDevice device,
  ...
  // Subpass 1 reads colorBuffer as input attachment, writes to swapchain (attachment 0)
...
// ---------------------------------------------------------------------------
void RenderPassManager::createShadowRenderPass(VkDevice device, VkFormat depthFormat) {
